...
# --- OS Detection for Fedora ---
if [ -f "/etc/fedora-release" ] || grep -q 'ID=fedora' /etc/os-release; then
...
while [[ $# -gt 0 ]]; do
    ...
    case $key in
        ...
        ;;
        -h|--help)
        echo "Usage: $0 [--build appimage] [--clean yes|no] [--test-flags]"
        echo "  --build: Specify the build format (appimage). Default: appimage"
...
# Force BUILD_FORMAT to appimage as .deb is not supported on Fedora
BUILD_FORMAT="appimage"
echo "Forcing build format to appimage as .deb is not supported on Fedora."
...
for cmd in $ALL_DEPS_TO_CHECK; do
    if ! check_command "$cmd"; then
        case "$cmd" in
            "p7zip") DEPS_TO_INSTALL="$DEPS_TO_INSTALL p7zip" ;; # Fedora package name
            "wget") DEPS_TO_INSTALL="$DEPS_TO_INSTALL wget" ;;
            "wrestool"|"icotool") DEPS_TO_INSTALL="$DEPS_TO_INSTALL icoutils" ;;
            "convert") DEPS_TO_INSTALL="$DEPS_TO_INSTALL ImageMagick" ;; # Fedora package name
...
const KeyboardKey = { Backspace: 43, Tab: 280, Enter: 261, Shift: 272, Control: 61, Alt: 40, CapsLock: 56, Escape: 85, Space: 276, PageUp: 251, PageDown: 250, End: 83, Home: 154, LeftArrow: 175, UpArrow: 282, RightArrow: 262, DownArrow: 81, Delete: 79, Meta: 187 };
Object.freeze(KeyboardKey);
module.exports = { getWindowsVersion: () => "10.0.0", setWindowEffect: () => {}, removeWindowEffect: () => {}, getIsMaximized: () => false, flashFrame: () => {}, clearFlashFrame: () => {}, showNotification: () => {}, setProgressBar: () => {}, clearOverlayIcon: () => {}, KeyboardKey };
EOF
...
if [ "$FINAL_OUTPUT_PATH" != "Not Found" ] && [ -e "$FINAL_OUTPUT_PATH" ]; then
    ...
    echo -e "\n🚀 To install AppImageLauncher on Fedora:"
    echo -e "   1. Add the copr repository:"
    echo -e "      \033[1;32msudo dnf copr enable fuhrmann/AppImageLauncher\033[0m"
    echo -e "   2. Install AppImageLauncher:"
    echo -e "      \033[1;32msudo dnf install AppImageLauncher\033[0m"
    echo -e "\n📦 To run the Claude AppImage after installing AppImageLauncher:"
    echo -e "   Make the AppImage executable and run it:"
    echo -e "   \033[1;32mchmod +x $FINAL_OUTPUT_PATH\033[0m"
    echo -e "   \033[1;32m$FINAL_OUTPUT_PATH\033[0m"
else
    echo -e "⚠️ AppImage file not found. Cannot provide installation instructions."
fi